...
class LibraryService:
    ...
    async def like(self, user_id: str, payload: LikedSongCreate) -> dict:
        result = await self.repository.like_song(user_id, payload)
        await self._invalidate_recommendations(user_id)
        return result

    async def unlike(self, user_id: str, provider: str, song_id: str) -> None:
        await self.repository.unlike_song(user_id, provider, song_id)
        await self._invalidate_recommendations(user_id)

    async def liked(self, user_id: str, limit: int) -> list[dict]:
        return await self.repository.liked_songs(user_id, limit)

    async def follow(self, user_id: str, payload: FollowedArtistCreate) -> dict:
        result = await self.repository.follow_artist(user_id, payload)
        await self._invalidate_recommendations(user_id)
        return result

    async def unfollow(self, user_id: str, provider: str, artist_id: str) -> None:
        await self.repository.unfollow_artist(user_id, provider, artist_id)
        await self._invalidate_recommendations(user_id)

    async def followed(self, user_id: str, limit: int) -> list[dict]:
