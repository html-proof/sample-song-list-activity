...
class PlaylistService:
    ...
    async def add_track(self, user_id: str, playlist_id: UUID, payload: PlaylistTrackCreate) -> dict:
        result = await self.repository.add_track(user_id, playlist_id, payload)
        await self._invalidate_recommendations(user_id)
        return result

    async def remove_track(self, user_id: str, playlist_id: UUID, track_id: UUID) -> None:
