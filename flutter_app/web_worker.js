// music-hub-web — single-origin worker.
// Serves the Flutter web build (static assets, SPA fallback) AND proxies the
// backend API to Render, so the app can use its own origin as the API base URL
// (no CORS, one URL). API paths are proxied with edge caching; everything else
// is served from the ASSETS binding.
...
// Per-path cache TTL (seconds). 0 = never cache (personalized / auth routes).
const TTL_RULES = [
  [/^\/me(?:\/|$)/, 0],
  [/^\/api\/(?:me|home)(?:\/|$)/, 0],
  [/^\/api\/pulse(?:\/|$)/, 0],
...
function isApiPath(pathname) {
  return API_PREFIXES.some((pattern) => pattern.test(pathname));
}
...
export default {
  async fetch(request, env) {
    ...
    // Everything else: the Flutter web app (static assets + SPA fallback).
